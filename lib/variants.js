...
function getType (options) {
  const type = options.type === 'windows' ? `windows${options.windows.protoclType}` : options.type;
...
function find (client) {
  return function find (options) {
    ...
    const req = {
      url: options.variantId ? `${client.baseUrl}/rest/applications/${options.pushAppId}/${type}/${options.variantId}` : `${client.baseUrl}/rest/applications/${options.pushAppId}/${type}`
    };
...
/**
  ...
  ** For Windows **
  @param {object} [options.windows] - The specific Windows options
  @param {string} [options.windows.protocolType] - mpns or wns
  @param {string} [options.windows.sid] - *for wns only* - (Package security identifier) used to connect to the windows push notification services
  @param {string} [options.windows.clientSecret] - *for wns only* - The client secret (password) to connect to the windows push notification services
  @returns {Promise} A promise that will resolve with the Array of variant objects or if a variant is specified, just the variant object
  @example
  adminClient(baseUrl, settings)
    .then((client) => {
...
function update (client) {
  return function update (options) {
    ...
    const req = {
      url: `${client.baseUrl}/rest/applications/${options.pushAppId}/${type}/${options.variantId}`,
      method: 'PUT'
    };
...
/**
  ...
  ** Windows Variant Only**
  @param {object} [options.windows]
  @param {string} [options.windows.protocalType] - mpns or wns
  @returns {Promise}  A promise that resolves with No Content.
  @example
  adminClient(baseUrl, settings)
    .then((client) => {
...
function reset (client) {
  return function reset (options) {
    ...
    const req = {
      url: `${client.baseUrl}/rest/applications/${options.pushAppId}/${type}/${options.variantId}/reset`,
      method: 'PUT'
    };
