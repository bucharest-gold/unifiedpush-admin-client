...
/**
  A function for exporting as JSON representation(gzip'd) of the installations for a given variant.
  @param {string} variantId - The id of the variant
  @returns {Promise} A promise that will resolve with the Array of installation objects
  @example
  adminClient(baseUrl, settings)
    .then((client) => {
      client.installations.exporter(variantId)
        .then((installations) => {
        console.log(installations) // [{...},{...}, ...]
      });
    });
...
function exporter(client) {
  return function exporter(variantId) {
    const req = {
      url: `${client.baseUrl}/rest/export/${variantId}/installations`,
      method: 'GET'
    };

    return request(client, req)
      .then((response) => {
        // API says there is only a 200 response
        return Promise.resolve(response.body);
      });
  };
}
